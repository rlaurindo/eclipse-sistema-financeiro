...
export const LoginGate: React.FC = () => {
  const { login, updatePassword, isPasswordRecovery, authConfigurationError } = useAuth();
    ...
    <main className="min-h-screen bg-slate-100 flex items-center justify-center p-4">
      <section className="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-7 shadow-xl">
        <div className="mb-7 text-center">
          ...
          <p className="mt-1 text-sm text-slate-500">{isPasswordRecovery ? 'Defina uma nova palavra-passe para a sua conta.' : 'Inicie sessão para aceder ao sistema.'}</p>
        </div>
        ...
        <form onSubmit={isPasswordRecovery ? handlePasswordUpdate : handleSubmit} className="space-y-4">
          {!isPasswordRecovery && (
          ...
          <button type="submit" disabled={loading || Boolean(authConfigurationError)} className="w-full rounded-xl bg-blue-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50">
            {loading ? 'A processar…' : isPasswordRecovery ? 'Guardar nova palavra-passe' : 'Entrar no sistema'}
          </button>
        </form>
      </section>
    </main>
